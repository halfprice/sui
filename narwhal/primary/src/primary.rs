...
use types::{
    ...
    now, validate_received_certificate_version, AggregateSignatureVerificationState, Certificate,
    CertificateAPI, CertificateDigest, FetchCertificatesRequest, FetchCertificatesResponse, Header,
    HeaderAPI, MetadataAPI, PreSubscribedBroadcastSender, PrimaryToPrimary, PrimaryToPrimaryServer,
    RequestVoteRequest, RequestVoteResponse, Round, SendCertificateRequest,
    SendCertificateResponse, Vote, VoteInfoAPI, WorkerOthersBatchMessage, WorkerOwnBatchMessage,
...
impl PrimaryToPrimary for PrimaryReceiverHandler {
    async fn send_certificate(
        ...
        validate_received_certificate_version(&mut certificate, &self.protocol_config).map_err(
            |err| {
                    ...
                    if self.protocol_config.narwhal_certificate_v2() {
                        ...
                        if !matches!(
                            ...
                            warn!(
                                ...
                                cert.digest(), cert.aggregate_signature_verification_state()
                            );
                            self.metrics
