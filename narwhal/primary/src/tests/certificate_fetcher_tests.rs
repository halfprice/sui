...
use types::{
    AggregateSignatureVerificationState, BatchDigest, Certificate, CertificateAPI,
    CertificateDigest, FetchCertificatesRequest, FetchCertificatesResponse, Header, HeaderAPI,
    HeaderDigest, Metadata, PreSubscribedBroadcastSender, PrimaryToPrimary, PrimaryToPrimaryServer,
...
async fn fetch_certificates_v1_basic() {
    let cert_v1_protocol_config = get_protocol_config(25);
...
async fn fetch_certificates_v2_basic() {
    ...
    for cert in certificates.iter().skip(num_written).take(204) {
        ...
        cert.set_aggregate_signature_verification_state(
            AggregateSignatureVerificationState::Unverified(AggregateSignatureBytes::default()),
        );
        certs.push(cert);
    }
    tx_fetch_resp
    ...
    // Send out a batch of certificates with good signatures for non-parents and
